...
def save_data(args,each_img,psnr,ssim):
    file = open("./radial_75_31_gamma=0.08_one/psnr_mask_radial{}_{}_gamma={}_one.txt".format(args.mask,each_img,args.gamma[0]), "w+")
...
def GlowDenoiser(args):
    ...
    for gamma in loopOver:
        for each_img in range(31):
	        ...
	        mask = io.loadmat('256_mask_under75.mat')['mask_radial75']#.format(img_shape,args.mask)   .format(args.mask)
	        ...
	        glow = Glow((1,img_shape,img_shape),
			            ...
			            device=args.device)
	        glow.load_state_dict(torch.load(modeldir+"/glowmodel_44000.pt"))
	        glow.eval()            
	        ...
	        for k in range(120):
		        ...
		        for i in range(1):
			        ...
			        with torch.no_grad():
			            ...
			            io.savemat('./radial_75_31_gamma=0.08_one/network_output_mask{}_{}_gamma={}_abs.mat'.format(args.mask,each_img,args.gamma[0]),{'rec':np.array(degrade,dtype=np.float32)})
			            sm.imsave('./radial_75_31_gamma=0.08_one/network_output_mask{}_{}_gamma={}_abs.png'.format(args.mask,each_img,args.gamma[0]),degrade)
			            degrade = torch.tensor(degrade,dtype=torch.float,requires_grad=False)
			            degrade = degrade.clone().to(device=args.device)
			            x_test[:,0:1,:,:] = degrade         #输入MRI图
			            #x_test[:,1,:,:] = degrade          #输入MRI图
