...
class Glow(nn.Module):
        ...
        if reverse:
            ...
            for i in range(len(self.glow_modules)-1,-1,-1 ):
                ...
                if  module_name == "Split":
                    ...
                    #print(len(self.glow_modules)-1,x_rev.shape,i,k)
                    k = k - 1
                elif  module_name == "Flow":
    ...
    def postprocess(self, x, floor_clamp=True):
        n_bins = 2 ** self.n_bits_x
        if floor_clamp:
            x[:,0,:,:] = torch.floor((x[:,0,:,:]+0.5)*n_bins)*(1./n_bins)
            x[:,1,:,:] = torch.floor((x[:,1,:,:]+0.5)*n_bins)*(1./n_bins) 
            x[:,2,:,:] = torch.floor((x[:,2,:,:])*n_bins)*(1./n_bins) 
            x[:,3,:,:] = torch.floor((x[:,3,:,:])*n_bins)*(1./n_bins) 
            x = torch.clamp(x, 0,1)
    ...
    def postprocess(self, x, floor_clamp=True):
        n_bins = 2 ** self.n_bits_x
        if floor_clamp:
            #x = torch.floor((x)*n_bins)*(1./n_bins) 
            x = torch.floor((x + 0.5)*n_bins)*(1./n_bins)
            x = torch.clamp(x, 0,1)
        else:
            x = x + 0.5
        return x
    def Dc(self,x_gen_complex,x_gen,kspace_torch,mask_torch):
    
        x_gen_complex[:,:,0] = torch.mean(x_gen[0,:,:,:],0)

        
        temp_FFT = torch.fft(x_gen_complex,2)
        #temp_FFT = torch.mean(temp_FFT,0)
        
        #print(temp_FFT.shape)
        #assert False
        temp_FFT[mask_torch==1] = kspace_torch[mask_torch==1]
        x_gen_mean = torch.ifft(temp_FFT,2)
        #print(x_gen_mean.shape)
        #assert False
        #print(x_gen.shape)
        x_gen[:,0,:,:] = x_gen_mean[:,:,0]
        x_gen[:,1,:,:] = x_gen_mean[:,:,0]
        x_gen[:,2,:,:] = x_gen_mean[:,:,0]
        
        #x_gen = torch.from_numpy(x_gen_mean_3.real)
        
        #x_gen = x_gen.unsqueeze(0).float().to('cuda')
        #del x_gen_complex,temp_FFT,kspace_torch,x_gen_mean
        return x_gen
