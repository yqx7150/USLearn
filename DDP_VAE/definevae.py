...
def definevae(lat_dim=60, patchsize=28,batchsize=50):
     ...
     #os.environ["CUDA_VISIBLE_DEVICES"] = '0';   #wsy add
     ...
     flat_relu3 = tf.contrib.layers.flatten(enc_relu3)                 #make input become a line data batchsize * LineData
     ...
     #==============================================================================
     #==============================================================================
     sess=tf.InteractiveSession(config=config)#config=config
     ...
     saver.restore(sess, './trained_model/cvae_MSJhalf_'+mode+'_fcl'+str(fcl_dim)+'_lat'+str(lat_dim)+'_ns'+str(noisy)+'_ps'+str(patchsize)+'54_'+"7.7")
